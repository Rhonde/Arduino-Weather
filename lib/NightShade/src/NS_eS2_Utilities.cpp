...
void NS_energyShield2::TWI_writeByte(uint8_t slaveAddress, uint8_t registerAddress, uint8_t data)
{
	nsWire->beginTransmission(slaveAddress);
	nsWire->write(registerAddress);
	nsWire->write(data);
	nsWire->endTransmission();
...
uint8_t NS_energyShield2::TWI_readByte(uint8_t slaveAddress, uint8_t registerAddress)
{
	uint8_t _data;
	nsWire->beginTransmission(slaveAddress);
	nsWire->write(registerAddress);
	nsWire->endTransmission(0); // Send restart
	nsWire->requestFrom(slaveAddress, 1u);
	_data = nsWire->read();
	delayMicroseconds(TWI_DELAY);
...
void NS_energyShield2::writeCommand(uint8_t slaveAddress, uint8_t registerAddress, uint16_t dataWord) {
  nsWire->beginTransmission(slaveAddress);
  nsWire->write(registerAddress);
  nsWire->write(dataWord & 0xFF);
  nsWire->write(dataWord >> 8);
  nsWire->endTransmission(); 
...
uint16_t NS_energyShield2::readCommand(uint8_t slaveAddress, uint8_t registerAddress) {
  ...
  nsWire->beginTransmission(slaveAddress);
  nsWire->write(registerAddress);
  nsWire->endTransmission(0);  
  nsWire->requestFrom(slaveAddress, 2u);
  dataWord = nsWire->read();
  dataWord += nsWire->read() << 8;
...
uint16_t NS_energyShield2::readSubCommand(uint8_t slaveAddress, uint16_t controlData) {
  ...
  nsWire->beginTransmission(slaveAddress);
  nsWire->write(0x00);
  nsWire->write(controlData & 0xFF);
  nsWire->write(controlData >> 8);
  nsWire->endTransmission();  
...
int NS_energyShield2::checkIfSealed(uint8_t slaveAddress) {
  uint16_t flags;
