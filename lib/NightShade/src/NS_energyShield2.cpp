...
void NS_energyShield2::setTimeDate(uint8_t second, uint8_t minute, uint8_t hour, uint8_t dayOfMonth, uint8_t dayOfWeek, uint8_t month, uint8_t year)
{
  ...
  nsWire->beginTransmission(RTC_SLAVE_ADDR);
  nsWire->write(0x04);
  for (int i = 0; i < 7; i++)
  {
    nsWire->write(timeDate[i]);
  }
...
void NS_energyShield2::readClock()
{
	...
	nsWire->requestFrom(RTC_SLAVE_ADDR,7);
	for (i=0;i<7;i++)
	{
		_timeDate[i] = nsWire->read();
	}
...
{
  nsWire->beginTransmission(RTC_SLAVE_ADDR);
  nsWire->write(0x0B); // Address of first alarm register
  for (int i = 0; i < 5; i++)
  {
    nsWire->write(0xFF); // Clear all alarms
  }
  nsWire->endTransmission();
...
void NS_energyShield2::writeAlarms(long alarmTimeSeconds) {
	...
	do {    
		...
		nsWire->write(0x0B);
		nsWire->endTransmission();
		nsWire->requestFrom(RTC_SLAVE_ADDR, 5);
		for (int i=0;i<5;++i) {
			received = nsWire->read();
...
int NS_energyShield2::readVMPP() {
	...
	do {
		nsWire->requestFrom(DAC_SLAVE_ADDR, 2);
		data[0] = nsWire->read();
		data[1] = nsWire->read();
		nsWire->endTransmission();
...
void NS_energyShield2::setVMPP(int MPP_Voltage_mV, bool writeEEPROM)
{
	...
	if (data[1] != DAC_setting)	{
		...
		// Write value to DAC
		nsWire->beginTransmission(DAC_SLAVE_ADDR);
		nsWire->write(Control);
		nsWire->write(Hbyte);
		nsWire->write(Lbyte);
		nsWire->endTransmission();
	}
...
int NS_energyShield2::begin(TwoWire *wire)
{
	int error=0;
